function [cluster_labels,k_centers] = k_means(data, centers, num_clusters,max_iter)
...
while 1
  ...
  if length(zero_cluster) > 0
    ...
    centers(zero_cluster, :)= orth_init(data, length(zero_cluster));
    cluster_size(zero_cluster) = 1;
    %if start again, then just set iter indicator to 0
    iter = 0;
